...
fn test_is_mutation_tool_classifies_correctly() {
    assert!(
        is_mutation_tool("mempalace_add_drawer"),
        "add_drawer should be mutation"
    );
    assert!(
        is_mutation_tool("mempalace_delete_drawer"),
        "delete_drawer should be mutation"
    );
    assert!(
        is_mutation_tool("mempalace_kg_add"),
        "kg_add should be mutation"
    );
    assert!(
        is_mutation_tool("mempalace_kg_invalidate"),
        "kg_invalidate should be mutation"
    );
    assert!(
        is_mutation_tool("mempalace_diary_write"),
        "diary_write should be mutation"
    );

    assert!(
        !is_mutation_tool("mempalace_status"),
        "status should be query"
    );
    assert!(
        !is_mutation_tool("mempalace_search"),
        "search should be query"
    );
    assert!(
        !is_mutation_tool("mempalace_kg_query"),
        "kg_query should be query"
    );
}
...
fn test_config_load_default_values() {
    ...
    assert!(
        !config.palace_path.to_string_lossy().is_empty(),
        "palace_path should be set"
    );
    assert!(
        !config.collection_name.is_empty(),
        "collection_name should be set"
    );
...
fn test_query_entity_returns_empty_for_nonexistent() {
    ...
    let results = kg
        .query_entity("nonexistent", None, None, "outgoing")
        .unwrap();
    assert!(
        results.is_empty(),
        "query_entity nonexistent should return empty vec"
    );
}
...
fn test_query_entity_returns_result_for_existing() {
    ...
    assert_eq!(
        results.len(),
        1,
        "query_entity Alice should return 1 result"
    );
...
fn test_query_entity_4arg_signature_stability() {
    ...
    let _ = kg
        .query_entity("test", Some("2025-01-01"), None, "both")
        .unwrap();
    let _ = kg
        .query_entity("test", None, Some("2024-06-01"), "incoming")
        .unwrap();
}
